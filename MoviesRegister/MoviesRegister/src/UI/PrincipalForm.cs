...
namespace MoviesRegister
{
    public partial class PrincipalForm : BaseForm
    {
        ...
        private Panel NovoPainel(int id, string title, string urlImagem, string releaseDate, string overview, Movie movie)
        {
        ...
        private void LocalizarFilmes()
        {
            ...
            foreach (MoviesHelper item in movies)
            {
                ...
                this.flpMovies.Controls.Add(NovoPainel(item.id, item.title.Length > 25 ? item.title.Substring(0, 25) + "..." : item.title, urlImagem + item.poster_path, item.release_date.ToString().Substring(0, 10),item.overview, movie));
        ...
        private void dgvFilmesFavoritos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
